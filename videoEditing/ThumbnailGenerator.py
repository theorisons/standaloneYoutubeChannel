...
class Thumbnail:
    def __init__(self, textInput, colorBackground, name):
        value = self._getMetric(textInput, usedFont)

        img = Image.new('RGB', sizeVideo, color=tuple(colorBackground))

        f = ImageFont.truetype(usedFont, value[1])
        d = ImageDraw.Draw(img)
        d.text(value[0], textInput, font=f, fill=colorText)

        img.save("{}/{}.png".format(folderDestination, name))
...
Thumbnail("Le corbeau et le renard", [255, 0, 0], "1_2")